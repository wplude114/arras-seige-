...
function drawEntityIcon(model, x, y, len, height, lineWidthMult, angle, alpha, colorIndex, upgradeKey, hover = false) {
    ...
    // Tank name
    drawText(picture.upgradeName ?? picture.name, x + (len / 2), y + height * 1.15, height / 10, color.guiwhite, "center");
}
...
function drawMinimapAndDebug(spacing, alcoveSize, GRAPHDATA) {
    ...
    // Text
    if (global.showDebug) {
        drawText(gameName, x + len, y - 50 - 5 * 14 - 2, 15, nameColor, "right");
        ...
        drawText(gameName, x + len, y - 50 - 2 * 14 - 2, 15, nameColor, "right");
